...
module {
  ...
  func.func @test_add_const_tensor_scalarize(%arg0: tensor<1x1x4x4x!ttype_f32, #layout>) -> tensor<1x1x4x4x!ttype_f32, #layout> {
    ...
    ^unified0(%cb0: !d2m.cb<tensor<4x4x!ttype_f32>>, %cb1: !d2m.cb<tensor<4x4x!ttype_f32>>, %cb2: !d2m.cb<tensor<4x4x!ttype_f32>>):
      ...
      %10 = d2m.remote_load %arg0[%iter0, %iter1] : tensor<1x1x4x4x!ttype_f32, #layout> -> tensor<4x4x!ttype_f32>
      %11 = d2m.remote_load %1[%iter0, %iter1] : tensor<1x1x4x4x!ttype_f32, #layout> -> tensor<4x4x!ttype_f32>
  ...
  func.func @test_scalarize_through_cast_and_view(%arg0: tensor<256x256xbf16, #ttnn.ttnn_layout<(d0, d1) -> (d0, d1), <8x8>, memref<1x1x!ttcore.tile<32x32, bf16>, #ttnn.buffer_type<l1>>, <block_sharded>, exactGrid = true>>) -> tensor<256x256xbf16, #ttnn.ttnn_layout<(d0, d1) -> (d0, d1), <8x8>, memref<1x1x!ttcore.tile<32x32, bf16>, #ttnn.buffer_type<l1>>, <block_sharded>, exactGrid = true>> {
    ...
    ^unified0(%cb0: !d2m.cb<tensor<8x8x!ttcore.tile<32x32, bf16>>>, %cb1: !d2m.cb<tensor<8x8x!ttcore.tile<32x32, bf16>>>, %cb2: !d2m.cb<tensor<8x8x!ttcore.tile<32x32, bf16>>>):
      ...
      %iter0 = d2m.iter_index(0) : index
      %iter1 = d2m.iter_index(1) : index
      %3 = d2m.remote_load %view[%iter0, %iter1] : tensor<1x1x8x8x!ttcore.tile<32x32, bf16>, #ttcore.metal_layout<logical_shape = 256x256, dim_alignments = 32x32, collapsed_intervals = dense<[[0, -1]]> : tensor<1x2xi64>, undef, l1, sharded, index_map = (d0, d1, d2, d3) -> (d0 * 8 + d2, d1 * 8 + d3, 0, 0)>> -> tensor<8x8x!ttcore.tile<32x32, bf16>>
      %4 = d2m.remote_load %view_0[%iter0, %iter1] : tensor<1x1x8x8x!ttcore.tile<32x32, bf16>, #ttcore.metal_layout<logical_shape = 256x256, dim_alignments = 32x32, collapsed_intervals = dense<[[0, -1]]> : tensor<1x2xi64>, undef, l1, sharded, index_map = (d0, d1, d2, d3) -> (d0 * 8 + d2, d1 * 8 + d3, 0, 0)>> -> tensor<8x8x!ttcore.tile<32x32, bf16>>
