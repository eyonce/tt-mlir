...
module {
  ...
  func.func @test_multicast_single_dim(%arg0: memref<2x4x2x4x!ttcore.tile<32x32, f32>, #ttcore.shard<16384x4096, 1>, #dram>) {
    ...
    ^unified0(%cb0: !d2m.cb<memref<2x4x!ttcore.tile<32x32, f32>, #l1_>>, %cb1: !d2m.cb<memref<2x4x!ttcore.tile<32x32, f32>, #l1_>>):
      ...
      // CHECK: d2m.remote_load %{{.*}}[%{{.*}}, %{{.*}}] into %cb0 mcore[%[[CORE0]], %[[C0]]] mshape[%[[C1]], %[[C4]]]
      d2m.remote_load %stream[%c0, %c1] into %cb0 mcast[%c1] : memref<2x4x2x4x!ttcore.tile<32x32, f32>, #ttcore.shard<16384x4096, 1>, #ttcore.view<map(4)>, #dram> into !d2m.cb<memref<2x4x!ttcore.tile<32x32, f32>, #l1_>>
    }
    return
  }
  ...
  func.func @test_multicast_first_dim(%arg0: memref<4x2x2x4x!ttcore.tile<32x32, f32>, #ttcore.shard<16384x4096, 1>, #dram>) {
    ...
    ^unified0(%cb0: !d2m.cb<memref<2x4x!ttcore.tile<32x32, f32>, #l1_>>, %cb1: !d2m.cb<memref<2x4x!ttcore.tile<32x32, f32>, #l1_>>):
      ...
      // CHECK: d2m.remote_load %{{.*}}[%{{.*}}, %{{.*}}] into %cb0 mcore[%[[C0]], %[[CORE1]]] mshape[%[[C4]], %[[C1]]]
      d2m.remote_load %stream[%c0, %c1] into %cb0 mcast[%c0] : memref<4x2x2x4x!ttcore.tile<32x32, f32>, #ttcore.shard<16384x4096, 1>, #ttcore.view<map(4)>, #dram> into !d2m.cb<memref<2x4x!ttcore.tile<32x32, f32>, #l1_>>
    }
    return
  }

  // Test lowering of high-level multicast form with both dimensions as multicast
  // Grid: 3x5 with iterator types [reduction, reduction]
  // Multicast dimensions: 0 and 1 (both reduction)
  // Expected: core[0, 0] mcast[3, 5]
  // CHECK-LABEL: func.func @test_multicast_both_dims
  func.func @test_multicast_both_dims(%arg0: memref<3x5x2x4x!ttcore.tile<32x32, f32>, #ttcore.shard<16384x4096, 1>, #dram>) {
    %alloc = memref.alloc() {alignment = 64 : i64} : memref<3x5x2x4x!ttcore.tile<32x32, f32>, #ttcore.shard<16384x4096, 1>, #l1_>
    %cb_alloc = memref.alloc() {alignment = 64 : i64} : memref<3x5x2x4x!ttcore.tile<32x32, f32>, #ttcore.shard<16384x4096, 1>, #l1_>
    %stream = "d2m.stream_layout"(%arg0, %cb_alloc) : (memref<3x5x2x4x!ttcore.tile<32x32, f32>, #ttcore.shard<16384x4096, 1>, #dram>, memref<3x5x2x4x!ttcore.tile<32x32, f32>, #ttcore.shard<16384x4096, 1>, #l1_>) -> memref<3x5x2x4x!ttcore.tile<32x32, f32>, #ttcore.shard<16384x4096, 1>, #ttcore.view<map(4)>, #dram>

    d2m.generic {block_factors = [1, 1], grid = #ttcore.grid<3x5>, indexing_maps = [#map, #map], iterator_types = [#reduction, #reduction], threads = [#d2m.thread<unified>]}
        ins(%stream : memref<3x5x2x4x!ttcore.tile<32x32, f32>, #ttcore.shard<16384x4096, 1>, #ttcore.view<map(4)>, #dram>)
        outs(%alloc : memref<3x5x2x4x!ttcore.tile<32x32, f32>, #ttcore.shard<16384x4096, 1>, #l1_>) {
    ^unified0(%cb0: !d2m.cb<memref<2x4x!ttcore.tile<32x32, f32>, #l1_>>, %cb1: !d2m.cb<memref<2x4x!ttcore.tile<32x32, f32>, #l1_>>):
      %c0 = arith.constant 0 : index
      %c1 = arith.constant 1 : index
      // High-level mcast form: mcast on both dims
      // CHECK-DAG: %[[C0:.*]] = arith.constant 0 : index
      // CHECK-DAG: %[[C3:.*]] = arith.constant 3 : index
      // CHECK-DAG: %[[C5:.*]] = arith.constant 5 : index
      // CHECK: d2m.remote_load %{{.*}}[%{{.*}}, %{{.*}}] into %cb0 mcore[%[[C0]], %[[C0]]] mshape[%[[C3]], %[[C5]]]
      d2m.remote_load %stream[%c0, %c1] into %cb0 mcast[%c0, %c1] : memref<3x5x2x4x!ttcore.tile<32x32, f32>, #ttcore.shard<16384x4096, 1>, #ttcore.view<map(4)>, #dram> into !d2m.cb<memref<2x4x!ttcore.tile<32x32, f32>, #l1_>>
    }
    return
  }
  ...
  func.func @test_multicast_unicast_strip(%arg0: memref<1x4x2x4x!ttcore.tile<32x32, f32>, #ttcore.shard<16384x4096, 1>, #dram>) {
    ...
    ^unified0(%cb0: !d2m.cb<memref<2x4x!ttcore.tile<32x32, f32>, #l1_>>, %cb1: !d2m.cb<memref<2x4x!ttcore.tile<32x32, f32>, #l1_>>):
      ...
      // CHECK-NOT: mcore[
      // CHECK-NOT: mshape[
      d2m.remote_load %stream[%c0, %c1] into %cb0 mcast[%c0] : memref<1x4x2x4x!ttcore.tile<32x32, f32>, #ttcore.shard<16384x4096, 1>, #ttcore.view<map(4)>, #dram> into !d2m.cb<memref<2x4x!ttcore.tile<32x32, f32>, #l1_>>
    }
    return
  }
}
